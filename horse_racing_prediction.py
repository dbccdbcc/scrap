...
# Get number of racedates available
...
if racedate_count < 400:
    ...
    SELECT * FROM race_results
    WHERE raceDateId >= (
        SELECT MAX(raceDateId) - 100 FROM race_results
    )
    """
...
def get_last_features(horse, date):
    prev = race_results[(race_results['horse'] == horse) & (race_results['race_date'] < date)]
...
for idx, row in tqdm(race_results.iterrows(), total=len(race_results), desc="  Historical Features"):
    feature_list.append(get_last_features(row['horse'], row['race_date']))
...
for idx, row in tqdm(future_races.iterrows(), total=len(future_races), desc="  Future Features"):
    future_feature_list.append(get_last_features(row['horse'], row['race_date']))
...
def monte_carlo_win_simulation(win_probs, n_sim=10000):
    ...
    if win_probs.sum() == 0:
        # Prevent division by zero if all win_probs are zero
        return results
    for _ in range(n_sim):
        # Each race: select a winner based on probabilities
...
for (race_id, race_df) in tqdm(race_groups, desc="Monte Carlo Races"):
    ...
    for horse, h_no, p_prob, w_prob, sim_p, sim_w in zip(
        horses, horse_nos, place_probs, win_probs, sim_place_pct, sim_win_pct
    ):
        output.append({
            'race_date': race_id[0],
            'race_no': race_id[1],
...
# Arrange columns to match your MySQL schema (add more as needed)
